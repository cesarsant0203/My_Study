...
-- de todos los productos que han tenido un número de unidades vendidas mayor o igual que el máximo de unidades 
-- vendidas en su categoría.
...
from PRODUCTOS pr join DETALLE_ORDENES de
on pr.PRODUCTOID = de.PRODUCTOID
where cantidad >= (
select max(cantidad)
from PRODUCTOS pro join DETALLE_ORDENES det
on pro.PRODUCTOID = det.PRODUCTOID
where pr.PRODUCTOID = pro.PRODUCTOID
...
/* 8. Cuente cuantos tipos distintos de libros se tiene. */
